...
from lib.trainingFunctions import *
import os
import numpy as np
...
def trainingFunc(xTrain,yTrain):
    ...
    if modelNo==5: #nikos1 clear fully connected
        model.add(Input(shape=inputShape))  ##input shape     
        model.add(Flatten()) ## matrix reshaping
        model.add(Dense(37))
        model.add(Dense(1))
        model.add(Activation('sigmoid'))
        print(model.summary())
    ...
    if modelNo==6:
        ...
        model.compile(optimizer='adam',loss='binary_crossentropy',metrics=['accuracy','mse']) ## loss='categorical_crossentropy' in case of multiclass
        model.summary()
        model.fit(xTrain,yTrain,epochs=epochsNum,validation_split=0.33,verbose=2,batch_size=batchSize)
